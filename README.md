...
The `/api/chat` route uses `ToolLoopAgent` with `createAgentUIStreamResponse()` and returns the selected model in the `x-openrouter-model` response header for local verification.
